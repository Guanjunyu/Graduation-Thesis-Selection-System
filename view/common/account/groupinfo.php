...
<body style="padding:10px; box-sizing: border-box;">
	...
	<table class="layui-table">
		...
		<tbody>
			...
			<tr>
				<td style="text-align:center;">{$vo.groupid}</td>
				<td style="text-align:center;">{$vo.groupname}</td>
				<td style="text-align:center;">{$vo.status==1?'开启':'<span style="color:red;">禁用</span>'}</td>
				<td style="text-align:center;">{$vo.right}</td>
				<td style="text-align:center;">
					<button type="button" class="layui-btn layui-btn-primary layui-btn-xs" onclick="edit({$vo.groupid})">
						<i class="layui-icon layui-icon-edit"></i>编辑
					</button>
					<button type="button" class="layui-btn layui-btn-primary layui-btn-xs" onclick="del({$vo.groupid})">
...
<script>
	...
	function edit(groupid){
		layer.open({
			...
			scrollbar:false,
			area: ['550px','550px'],
			content: "groupedit?groupid="+groupid
		});
	}
	...
	function del(groupid){
		layer.confirm('确定要删除吗？', {
			...
			$.post("groupdel",{'groupid':groupid},function(res){
				if(res.code!=200){
