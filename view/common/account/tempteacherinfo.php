	...
	<table class="layui-table">
		...
		<tbody>
			...
			<tr>
				...
				<td style="text-align:center;">
					{if $vo.checked==0}
					<button type="button" class="layui-btn layui-btn-primary layui-btn-xs" onclick="pass({$vo.teacherid})">
						<i class="layui-icon layui-icon-edit"></i>通过
					</button>
					<button type="button" class="layui-btn layui-btn-primary layui-btn-xs" onclick="nopass({$vo.teacherid})">
...
<script>
	...
	function pass(teacherid){
		layer.confirm('确定要审核通过吗？', {
			...
			$.post("tempteacheradd",{'teacherid':teacherid},function(res){
				if(res.code!=200){
	...
	//不通过
	function nopass(teacherid){
		layer.confirm('确定要审核不通过吗？', {
			icon:3,
			btn: ['确定','取消']
		}, function(){
			$.post("tempteachernopass",{'teacherid':teacherid},function(res){
				if(res.code!=200){
					layer.alert(res.msg,{icon:2});
				}else{
					layer.msg(res.msg);
	...
	function del(teacherid){
		layer.confirm('确定要删除吗？', {
			...
			$.post("tempteacherdel",{'teacherid':teacherid},function(res){
				if(res.code!=200){
					layer.alert(res.msg,{icon:2});
				}else{
					layer.msg(res.msg);
