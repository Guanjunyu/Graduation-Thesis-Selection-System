...
<body style="padding:10px; box-sizing: border-box;">
	...
	<input type="hidden" name="studentid" value="{$lists.studentid}">

	<!-- 不允许修改 -->
	<div class="layui-form-item">
		<label class="layui-form-label">账户</label>
		<div class="layui-input-block">
			<input type="text" class="layui-input" value="{$lists.studentid}" readonly style="background-color:snow;">
		</div>
	</div>
...
<script>
	...
	layui.use(['form','layer'],function name() {
		...
		form.on('submit(save)', function (data) {
			...
			$.ajax({
				type: "post",
				url: "{:url('../../common/Account/studentedit')}",
				data: data.field,
				dataType: 'json',
