	...
	<table class="layui-table">
		...
		<tbody>
			...
			<tr>
				...
				<td style="text-align:center;">
					<button type="button" class="layui-btn layui-btn-primary layui-btn-xs" onclick="edit({$vo.adminid})">
						<i class="layui-icon layui-icon-edit"></i>编辑
					</button>
					<button type="button" class="layui-btn layui-btn-primary layui-btn-xs" onclick="del({$vo.adminid})">
...
<script>
	...
	function edit(adminid){
		layer.open({
			...
			anim: 1,
			area: ['550px','550px'],
			content: "adminedit?adminid="+adminid
		});
	}
	...
	function del(adminid){
		layer.confirm('确定要删除吗？', {
			...
			$.post("admindel",{'adminid':adminid},function(res){
				if(res.code!=200){
