	...
	<table class="layui-table">
		...
		<tbody>
			...
			<tr>
				...
				<td style="text-align:center;">{$vo.checked==0?'<span style="color:red;">未审核</span>':'审核通过'}</td>
                <td style="text-align:center;">
                    {if $vo.checked==0}
					<button type="button" class="layui-btn layui-btn-primary layui-btn-xs" onclick="pass({$vo.Id})">
...
<script>
	...
	function pass(Id){
		layer.confirm('确定要审核通过吗？', {
			...
			$.post("auditprojectadd",{'Id':Id},function(res){
				if(res.code!=200){
	...
	function del(Id){
		layer.confirm('确定要审核不通过吗？', {
			...
			$.post("auditprojectdel",{'Id':Id},function(res){
				if(res.code!=200){
					layer.alert(res.msg,{icon:2});
				}else{
					layer.msg(res.msg);
