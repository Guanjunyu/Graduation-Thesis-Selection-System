...
<body>
  ...
  <form class="layui-form layui-form-pane">
    ...
    <div class="layui-form-item layui-form-text">
      ...
      <div class="layui-input-block">
        <textarea class="layui-textarea" readonly style="background-color:snow;">{$data.description}</textarea>
      </div>
    </div>
    ...
    <div class="layui-form-item layui-form-text">
      ...
      <div class="layui-input-block">
        <textarea class="layui-textarea" readonly style="background-color:snow;">{$data.content}</textarea>
      </div>
    </div>
      ...
      <div class="layui-form-item">
          ...
          <button type="button" class="layui-btn layui-btn-primary" onclick="del({$data.studentid},{$data.projectid})">
						<i class="layui-icon layui-icon-delete"></i>退选
					</button>
			</div>
		  </div>
</center>
...
layui.use(['form','layer'], function name() {
  	...
  	// 删除
	function del(studentid,projectid){
		layer.confirm('确定要退选吗？', {
			icon:3,
			btn: ['确定','取消']
		}, function(){
			$.post('showsprojectstudent',{'studentid':studentid,'projectid':projectid},function(res){
				if(res.code!=200){
