...
<body style="padding:10px; box-sizing: border-box;">
	<!-- 头部 -->
	<div class="header">
		<a href="menuinfo"><span style="background:#999;">导航列表</span></a>
		<span>菜单列表</span>
		<button type="button" class="layui-btn layui-btn-sm layui-btn-primary" onclick="add({$smid})">
	...
	<table class="layui-table">
		<thead>
			<tr>
				<th  style="text-align:center;">排序/权重</th>
		...
		<tbody>
			...
			<tr>
				...
				<td  style="text-align:center;">{$vo['lable']}</td>
				<td  style="text-align:center;">{$vo['type_name']}</td>
				<td  style="text-align:center;">{$vo['src']}</td>
				<td  style="text-align:center;">{$vo['status']==1?'开启':'<span style="color:red;">禁用</span>'}</td>
...
<script>
	...
	function add(smid){
		layer.open({
			...
			shade: 0.3,
			area: ['550px','550px'],
			content: '../../Common/navigation/buttonadd?smid='+smid
		});
	}
	...
	function edit(smid){
		layer.open({
			...
			shade: 0.3,
			area: ['550px','550px'],
			content: '../../Common/navigation/buttonedit?smid='+smid
		});
	}
	...
	function del(smid){
		layer.confirm('确定要删除吗？', {
			...
			$.post('../../Common/navigation/buttondel',{'smid':smid},function(res){
				if(res.code!=200){
